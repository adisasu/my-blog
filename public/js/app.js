...
$( document ).ready(function() {
    function getArticles() {
        $.ajax({
            url: "http://localhost/blogg/admin/getJson",
    ...
    $('#articlesTbl').on('click', '[data-edit-id]', function() {
        ...
        $.ajax({
            url: "http://localhost/blogg/admin/getArticle?id=" + $(this).data('edit-id'),
            success: function( data ) {
    ...
    $('#articlesTbl').on('click', '[data-delete-id]', function() {
        //console.log($(this).data('edit-id'));
        $.ajax({
            url: "http://localhost/blogg/admin/deleteArticle?id=" + $(this).data('delete-id'),
            method : 'DELETE',
    ...
    $('input[type=submit]').on('click', function() {
      var articleForm = $('#articleForm');
        if ( $('input[name=id]').val()  !== '') {
            $.ajax({
                url: "http://localhost/blogg/admin/updateArticle",
            ...
            $.ajax({
                url: "http://localhost/blogg/admin/addArticle",
                data: $('#articleForm').serialize(),
                method: 'POST',
                success: function(data) {
                    articleForm[0].reset();
